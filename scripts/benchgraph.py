...
def main():
    ...
    for i, spec in enumerate(specs, 1):
        ...
        rows.append({"name": name, **pr, "ours_st": ours[0], "ours_t": ours[1],
                     "base_st": base[0], "base_t": base[1]})
        if i % 50 == 0 or i == total:
            el = time.monotonic() - t0
            print(f"[{i}/{total}] {el:.0f}s elapsed", file=sys.stderr, flush=True)

    with open(data_path, "w") as f:
        f.write("name\tself_contained\tabs_clusters\tresidual_class\tours_st\tours_t\tbase_st\tbase_t\n")
        for r in rows:
            f.write(f"{r['name']}\t{int(r['self_contained'])}\t{r['abs_clusters']}\t"
                    f"{r['residual_class']}\t{r['ours_st']}\t{r['ours_t'] or ''}\t"
