...
static bool timed_out(const Checker *ck) {
  if (ck->options.cancelled && ck->options.cancelled(ck->options.cancel_ctx))
    return true;
  if (ck->options.deadline_mono_ns &&
      now_seconds() * 1e9 >= (double)ck->options.deadline_mono_ns)
    return true;
...
static bool validate_sidecars(Checker *ck, char *message, size_t cap) {
  char *policy = ck->options.policy_json_text
                     ? strdup(ck->options.policy_json_text)
                     : read_text_file(ck->options.policy_json_path);
  if (!policy) {
  ...
  if (!ck->options.certificate_path)
    return true;
  char *certificate = ck->options.certificate_json_text
                          ? strdup(ck->options.certificate_json_text)
                          : read_text_file(ck->options.certificate_json_path);
  if (!certificate) {
...
static bool setup_bdds(Checker *ck, const uint32_t *levels, char *message,
  ...
  ck->bdd_options.deadline_mono_ns = ck->options.deadline_mono_ns;
  ck->bdd_options.cancelled = ck->options.cancelled;
  ck->bdd_options.cancel_ctx = ck->options.cancel_ctx;
...
static bool emit_controller(Checker *ck, const char *path) {
  ...
  Bdd2Aig conversion = {.strat = controller,
                        .var2lit = var2lit,
                        .var_count = ck->nvars,
                        .budget_run = &ck->run};
  for (uint32_t i = 0; i < ck->nc; i++) {
...
static ssize_t bounded_result_write(void *opaque, const char *data,
                                    size_t count) {
  BoundedResult *result = opaque;
  if (result->failed || count > result->limit - result->used) {
    result->failed = true;
    if (result->limited)
      *result->limited = true;
    errno = EFBIG;
  ...
  size_t needed = result->used + count + 1;
  if (needed > result->capacity) {
    size_t capacity = result->capacity ? result->capacity : 256;
    size_t maximum = result->limit + 1;
    if (capacity > maximum)
      capacity = maximum;
    while (capacity < needed)
      capacity = capacity > maximum / 2 ? maximum : capacity * 2;
    char *grown = realloc(result->data, capacity);
    if (!grown) {
      result->failed = true;
  ...
  memcpy(result->data + result->used, data, count);
  result->used += count;
  return (ssize_t)count;
}

static int bounded_result_close(void *opaque) {
  BoundedResult *result = opaque;
  if (result->failed) {
    free(result->data);
    free(result);
  ...
  result->data[result->used] = '\0';
  *result->output = result->data;
  *result->size = result->used;
  free(result);
  return 0;
}
    ...
    defined(__NetBSD__)
static int bounded_result_write_bsd(void *opaque, const char *data, int count) {
  return (int)bounded_result_write(opaque, data, (size_t)count);
}
#endif

static FILE *open_result_stream(const Options *options) {
  if (!options->json_bytes)
    return fopen(options->json_out_path, "w");
  *options->json_bytes = nullptr;
  *options->json_size = 0;
  if (!options->max_artifact_bytes || options->max_artifact_bytes == SIZE_MAX)
    return open_memstream(options->json_bytes, options->json_size);
  BoundedResult *result = oxidd_host_calloc(1, sizeof *result);
  if (!result) {
    if (options->json_allocation_failed)
      *options->json_allocation_failed = true;
    return nullptr;
  }
  result->output = options->json_bytes;
  result->size = options->json_size;
  result->limit = options->max_artifact_bytes;
  result->limited = options->json_limited;
  result->allocation_failed = options->json_allocation_failed;
#if defined(__APPLE__) || defined(__FreeBSD__) || defined(__OpenBSD__) ||      \
    defined(__NetBSD__)
  FILE *stream = funopen(result, nullptr, bounded_result_write_bsd, nullptr,
                         bounded_result_close);
#else
  cookie_io_functions_t io = {.write = bounded_result_write,
                              .close = bounded_result_close};
  FILE *stream = fopencookie(result, "w", io);
#endif
  if (!stream) {
    if (options->json_allocation_failed)
      *options->json_allocation_failed = true;
...
static bool write_result_json(const Checker *ck,
                              ...
                              double elapsed) {
  if (!ck->options.json_out_path && !ck->options.json_bytes)
    return true;
...
static Aig *native_parse_aag(const uint8_t *bytes, size_t size) {
  if (!bytes || !size || memchr(bytes, 0, size))
    return nullptr;
  FILE *in = fmemopen((void *)bytes, size, "r");
  if (!in)
    return nullptr;
...
static int run_options(Options options, char *owned_policy_json,
  ...
  if (options.policy_path)
    ck.policy =
        options.policy_bytes
            ? native_parse_aag(options.policy_bytes, options.policy_size)
            : read_aag(options.policy_path, false, message, sizeof message);
  if (options.certificate_path)
    ck.certificate = options.certificate_bytes
                         ? native_parse_aag(options.certificate_bytes,
  ...
  if (certificate.result != CHECK_SKIPPED)
    if (!options.quiet)
      printf("METHOD certificate %s time=%.6f\n",
             result_name(certificate.result), certificate.seconds);
  if (closed_loop.result != CHECK_SKIPPED)
    if (!options.quiet)
      printf("METHOD closed-loop %s time=%.6f\n",
             result_name(closed_loop.result), closed_loop.seconds);
...
int tlsf_certcheck_cli_main(int argc, char **argv) {
  ...
  return run_options(options, owned_policy_json, owned_cert_json, nullptr,
                     nullptr);
}

static TlsfGr1CheckStatus check_failure(TlsfGr1CheckResult *out,
                                        TlsfGr1CheckStatus status,
                                        const char *stage,
                                        const char *message) {
  out->status = status;
  out->verdict = TLSF_GR1_CHECK_UNKNOWN;
  snprintf(out->stage, sizeof out->stage, "%s", stage);
  snprintf(out->message, sizeof out->message, "%s", message);
  return status;
}

static bool valid_span(TlsfGr1Bytes span) {
  return span.data && span.size && !memchr(span.data, 0, span.size);
}

static char *copy_span(TlsfGr1Bytes span) {
  if (span.size == SIZE_MAX)
    return nullptr;
  char *copy = malloc(span.size + 1);
  if (copy) {
    memcpy(copy, span.data, span.size);
    copy[span.size] = 0;
  }
  return copy;
}
...
TlsfGr1CheckStatus tlsf_gr1_check(const TlsfGr1CheckInput *input,
                                  ...
                                  TlsfGr1CheckResult *out) {
  if (!out)
    return TLSF_GR1_CHECK_BAD_ARGUMENT;
  memset(out, 0, sizeof *out);
  out->verdict = TLSF_GR1_CHECK_UNKNOWN;
  bool region = options && options->method == TLSF_GR1_CHECK_REGION;
  bool needs_certificate =
      options && (options->method == TLSF_GR1_CHECK_CERTIFICATE ||
                  options->method == TLSF_GR1_CHECK_BOTH || region);
  ...
  if (!input || !options || options->abi_version != 1 ||
      ...
      (!has_certificate &&
       (input->certificate_aag.data || input->certificate_aag.size ||
        input->certificate_json.data || input->certificate_json.size)) ||
      (!region &&
       (!valid_span(input->policy_aag) || !valid_span(input->policy_json))) ||
      (region && (input->policy_aag.data || input->policy_aag.size ||
                  input->policy_json.data || input->policy_json.size)) ||
      !options->node_cap || !options->cache_cap || !options->max_artifact_bytes)
    return check_failure(out, TLSF_GR1_CHECK_BAD_ARGUMENT, "check",
                         "invalid argument or missing resource cap");
  if (input->game_aag.size > options->max_artifact_bytes ||
      input->certificate_aag.size > options->max_artifact_bytes ||
      input->certificate_json.size > options->max_artifact_bytes ||
      input->policy_aag.size > options->max_artifact_bytes ||
      input->policy_json.size > options->max_artifact_bytes)
    return check_failure(out, TLSF_GR1_CHECK_LIMIT, "check",
                         "input artifact cap exceeded");
  if (options->cancelled && options->cancelled(options->cancel_ctx))
    return check_failure(out, TLSF_GR1_CHECK_CANCELLED, "check", "cancelled");
  if (options->deadline_mono_ns &&
      now_seconds() * 1e9 >= (double)options->deadline_mono_ns)
    return check_failure(out, TLSF_GR1_CHECK_DEADLINE, "check",
                         "deadline exceeded");

  char *policy_json = region ? nullptr : copy_span(input->policy_json);
  char *certificate_json =
      has_certificate ? copy_span(input->certificate_json) : nullptr;
  if ((!region && !policy_json) || (has_certificate && !certificate_json)) {
    free(policy_json);
    free(certificate_json);
    return check_failure(out, TLSF_GR1_CHECK_LIMIT, "check",
                         "sidecar allocation failed");
  }
  Method method = METHOD_CERTIFICATE;
  switch (options->method) {
  case TLSF_GR1_CHECK_AUTO:
    method = METHOD_AUTO;
    break;
  case TLSF_GR1_CHECK_CERTIFICATE:
    method = METHOD_CERTIFICATE;
    break;
  case TLSF_GR1_CHECK_REGION:
    method = METHOD_REGION;
    break;
  case TLSF_GR1_CHECK_CLOSED_LOOP:
    method = METHOD_CLOSED_LOOP;
    break;
  case TLSF_GR1_CHECK_BOTH:
    method = METHOD_BOTH;
    break;
  }
  bool json_limited = false, json_allocation_failed = false;
  Options run = {
      .game_path = "memory",
      .policy_path = region ? nullptr : "memory",
      .policy_json_path = region ? nullptr : "memory",
      ...
      .game_bytes = input->game_aag.data,
      .game_size = input->game_aag.size,
      .policy_bytes = input->policy_aag.data,
      .policy_size = input->policy_aag.size,
      .certificate_bytes = input->certificate_aag.data,
      .certificate_size = input->certificate_aag.size,
      .policy_json_text = policy_json,
      .certificate_json_text = certificate_json,
      .json_bytes = &out->json,
      .json_size = &out->json_size,
      .max_artifact_bytes = options->max_artifact_bytes,
      .json_limited = &json_limited,
      .json_allocation_failed = &json_allocation_failed,
      .deadline_mono_ns = options->deadline_mono_ns,
      .cancelled = options->cancelled,
      .cancel_ctx = options->cancel_ctx,
      .quiet = true,
  };
  if (!effective_oxidd_node_capacity(run.node_cap, &run.effective_node_cap) ||
      !effective_oxidd_cache_capacity(run.cache_cap,
                                      &run.effective_cache_cap) ||
      run.effective_cache_cap > run.effective_node_cap) {
    free(policy_json);
    free(certificate_json);
    return check_failure(out, TLSF_GR1_CHECK_BAD_ARGUMENT, "check",
                         "invalid checker caps");
  }
  OxiddFailure failure = {0};
  int code = run_options(run, policy_json, certificate_json, &out->peak_nodes,
                         &failure);
  if (failure.kind == OXIDD_FAILURE_CANCELLED ||
      failure.kind == OXIDD_FAILURE_DEADLINE) {
    tlsf_gr1_check_result_clear(out);
    return check_failure(out,
                         failure.kind == OXIDD_FAILURE_CANCELLED
                             ? TLSF_GR1_CHECK_CANCELLED
                             : TLSF_GR1_CHECK_DEADLINE,
                         failure.phase ? failure.phase : "construction",
                         "checker stopped during BDD construction");
  }
  if (options->cancelled && options->cancelled(options->cancel_ctx)) {
    tlsf_gr1_check_result_clear(out);
    return check_failure(out, TLSF_GR1_CHECK_CANCELLED, "check", "cancelled");
  ...
  if ((code == EXIT_UNKNOWN || code == EXIT_ERROR) &&
      (failure.kind == OXIDD_FAILURE_BDD ||
       failure.kind == OXIDD_FAILURE_HOST)) {
    tlsf_gr1_check_result_clear(out);
    return check_failure(out, TLSF_GR1_CHECK_LIMIT, "check",
                         "checker allocation or node cap exhausted");
  }
  if (json_limited) {
    tlsf_gr1_check_result_clear(out);
    return check_failure(out, TLSF_GR1_CHECK_LIMIT, "result",
                         "result artifact cap exceeded");
  }
  if (json_allocation_failed) {
    tlsf_gr1_check_result_clear(out);
    return check_failure(out, TLSF_GR1_CHECK_LIMIT, "result",
                         "result allocation failed");
  }
  if (code == EXIT_INTERNAL || code == EXIT_ERROR) {
    tlsf_gr1_check_result_clear(out);
    return check_failure(out, TLSF_GR1_CHECK_ERROR, "check",
                         "checker internal or output error");
  }
  if (!out->json || out->json_size > options->max_artifact_bytes) {
    tlsf_gr1_check_result_clear(out);
    return check_failure(out, TLSF_GR1_CHECK_LIMIT, "check",
                         "result allocation or artifact cap exceeded");
  }
  switch (code) {
  case EXIT_VERIFIED:
    out->verdict =
        region ? TLSF_GR1_CHECK_REGION_VERIFIED : TLSF_GR1_CHECK_VERIFIED;
    break;
  case EXIT_CERT_FAILED:
    out->verdict = TLSF_GR1_CHECK_CERT_FAILED;
    break;
  case EXIT_REFUTED:
    out->verdict = TLSF_GR1_CHECK_REFUTED;
    break;
  case EXIT_INVALID:
    out->verdict = TLSF_GR1_CHECK_INVALID;
    break;
  default:
    out->verdict = TLSF_GR1_CHECK_UNKNOWN;
    break;
  }
  out->status = TLSF_GR1_CHECK_OK;
  snprintf(out->stage, sizeof out->stage, "check");
  return TLSF_GR1_CHECK_OK;
}
