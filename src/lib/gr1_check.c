...
bool gr1_check_node_capacity(size_t requested, size_t *effective) {
  /*
...
bool gr1_check_cache_capacity(size_t requested, size_t *effective) {
  /*
...
TlsfGr1CheckStatus tlsf_gr1_check(const TlsfGr1CheckInput *input,
  ...
  if (!gr1_check_node_capacity(run.node_cap, &run.effective_node_cap) ||
      !gr1_check_cache_capacity(run.cache_cap, &run.effective_cache_cap) ||
  ...
  int code = gr1_check_run(run, policy_json, certificate_json, &out->peak_nodes,
                           &failure);
