...
static unsigned tpl_bit(const char *s) {
  ...
  if (!strcmp(s, "global-recurrence-switch"))
    return TPL_GLOBAL_RECURRENCE;
  if (!strcmp(s, "mutex"))
    return TPL_MUTEX;
  if (!strcmp(s, "arbiter"))
    return TPL_ARBITER;
...
#define NEED_ARG()                                                             \
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (strcmp(a, "--candidates") == 0) {
      ...
      if (!cli_parse_param(v, "tlsftemplates", &overrides[n_overrides++]))
        return 1;
...
#undef NEED_ARG
  ...
  if (!fp)
    return 1;
  TlsfSpec *spec = spec_parse(fp, "tlsftemplates");
  if (input_file)
    fclose(fp);
  if (!spec)
    return 1;
