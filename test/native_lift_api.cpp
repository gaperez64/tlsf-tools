...
static const char *source =
    "INFO { TITLE: \"unseen\" DESCRIPTION: \"unseen\" SEMANTICS: Mealy TARGET: Mealy }\n"
...
static const char *nonparam =
    "INFO { TITLE: \"plain\" DESCRIPTION: \"plain\" SEMANTICS: Mealy TARGET: Mealy }\n"
    "MAIN { INPUTS { a; } OUTPUTS { b; } GUARANTEES { G F b; } }\n";
static const char *nonparam_moore =
    "INFO { TITLE: \"plain\" DESCRIPTION: \"plain\" SEMANTICS: Moore TARGET: Moore }\n"
    "MAIN { INPUTS { a; } OUTPUTS { b; } GUARANTEES { G F b; } }\n";
static const char *encoded_width =
    "INFO { TITLE: \"width\" DESCRIPTION: \"width\" SEMANTICS: Mealy TARGET: Mealy }\n"
...
static const char *large_arity =
    "INFO { TITLE: \"arity\" DESCRIPTION: \"arity\" SEMANTICS: Mealy TARGET: Mealy }\n"
...
static TlsfGr1LiftOptions options() {
  ...
  o.schema_nodes=1u<<18;o.schema_cache=1u<<16;
  o.max_monitor_states=1000;o.max_artifact_bytes=4u<<20;
  return o;
}
...
int main() {
  ...
  assert(fp);
  std::unique_ptr<Aig,decltype(&aig_free)> mutated(aig_read_aag(fp),&aig_free);
  fclose(fp);assert(mutated);
  aig_set_output(mutated.get(),"inv",AIG_FALSE);
  char *bytes=nullptr;size_t size=0;
  fp=open_memstream(&bytes,&size);assert(fp);
  aig_write_aag(fp,mutated.get());assert(fclose(fp)==0);
  ...
  input.certificate_json={(const uint8_t *)result.certificate_json,result.certificate_json_size};
  ...
  auto rename_all=[&](const std::string &from,const std::string &to){
    ...
    while ((at=renamed.find(from,at))!=std::string::npos) {
      renamed.replace(at,from.size(),to);at+=to.size();
    }
  };
  ...
  tlsf_gr1_lift_result_clear(&result);
  o=options();o.policy_proof_fraction=1e-12;
  assert(lift(source,o,&result,&error)==TLSF_GR1_LIFT_OK);
  ...
  o=options();o.cancelled=mutate_after_snapshot;o.cancel_ctx=&mutation;
