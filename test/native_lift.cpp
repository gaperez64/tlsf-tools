...
int main(int argc, char **argv) {
  if (argc < 2 || argc > 4) {
    std::cerr << "usage: native_lift INPUT [OUTPUT-PREFIX] [SECONDS]\n";
    return 2;
  }
  ...
  options.solver_nodes = 1u << 18;
  options.solver_cache = 1u << 16;
  options.checker_nodes = 1u << 18;
  options.checker_cache = 1u << 16;
  options.schema_nodes = 1u << 18;
  options.schema_cache = 1u << 16;
