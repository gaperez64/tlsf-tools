...
int main(void) {
  ...
  TlsfPipelineOptionsV2 load_options = {
      .certify = true,
  ...
  TlsfGr1ReductionOptions options = {
      .abi_version = TLSF_GR1_REDUCTION_ABI_VERSION,
      .struct_size = sizeof(TlsfGr1ReductionOptions),
  ...
  assert(tlsf_gr1_reduce_v1(pipeline, &options, &result, &error) ==
         TLSF_GR1_REDUCE_OK);
  ...
  pipeline = tlsf_pipeline_load_bytes_v2(
      (const uint8_t *)fallback_source, strlen(fallback_source), &load_options);
  assert(pipeline);
